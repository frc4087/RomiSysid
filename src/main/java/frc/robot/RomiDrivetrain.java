   ...
   public void logEntry(SysIdRoutineLog log) {
    log.motor("drive-left")
        .voltage(_dummyVoltage.mut_replace(
            ...
            Units.Volts))
        .linearPosition(_dummyDistance.mut_replace(m_leftEncoder.getDistance(),
            Units.Meters))
        .linearVelocity(_dummyVelocity.mut_replace(m_leftEncoder.getRate(),
            Units.MetersPerSecond));
    log.motor("drive-right")
        .voltage(_dummyVoltage.mut_replace(m_rightMotor.get() * RobotController
            .getBatteryVoltage(), Units.Volts))
        .linearPosition(_dummyDistance.mut_replace(m_rightEncoder.getDistance(),
            Units.Meters))
        .linearVelocity(_dummyVelocity.mut_replace(m_rightEncoder.getRate(),
            Units.MetersPerSecond));
  }
